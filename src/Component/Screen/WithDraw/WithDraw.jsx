...
function WithDraw() {
  ...
  useEffect(() => {
    if (loggedInUID) {
      ...
      const WithdrawRef = ref(db, "Withdraw");
  ...
  const handleSaveChanges = async () => {
    if ((TransactionDate && Amount && Note) || (CheckOnline && SelectedBank)) {
      ...
      try {
        debugger;
        const loggedInUID = localStorage.getItem("uid");
        const WithdrawRef = ref(db, `Withdraw/${editedItem}`);
        const newWithdraw = {
          uid: loggedInUID,
        ...
        update(WithdrawRef, newWithdraw);
        ...
        toast.error("Error adding Withdraw: " + error.message, {
          position: "top-right",
          autoClose: 1000,
          hideProgressBar: false,
          closeOnClick: true,
  ...
  const handleConfirmDelete = async () => {
    try {
      const dataRef = ref(db, `Withdraw/${id}`);
      ...
      <Main>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Edit WithDraw</Modal.Title>
          </Modal.Header>
          <Modal.Body>
